...
from glorn.modules.layers import build_dropout_layer
from glorn.modules.attention.output_layer import AttentionOutput
...
class Overlapping(nn.Module):
    def __init__(self, mode, d_model, num_heads, dropout=None, activation_fn='ReLU'):
        super(Overlapping, self).__init__()
        self.mode = mode
        if mode == "global patchs":
            self.cross_layer = nn.Sequential(
                nn.Conv1d(d_model * 3, d_model, kernel_size=1, stride=1, bias=False),
                nn.InstanceNorm1d(d_model),
                nn.LeakyReLU(0.1),
                nn.Conv1d(d_model, d_model, kernel_size=1, stride=1, bias=False),
                nn.InstanceNorm1d(d_model),
                nn.LeakyReLU(0.1),
            )
            self.overlap_layer = nn.Conv1d(d_model, 1, kernel_size=1, stride=1)
        else:
            self.attention = AttentionLayer(d_model, num_heads, dropout=dropout)
            self.output = AttentionOutput(d_model, dropout=dropout, activation_fn=activation_fn)
    ...
    def forward(
        ...
        if self.mode == "global patchs":
            src_feats, tgt_feats = input_states, memory_states
            src_global, tgt_global = src_feats.max(2)[0], tgt_feats.max(2)[0]
            src_cross, tgt_cross = torch.cat([
                src_global.unsqueeze(2).repeat(1, 1, src_feats.shape[2]),
                src_feats,
                tgt_global.unsqueeze(2).repeat(1, 1, src_feats.shape[2])
            ], 1), torch.cat([
                tgt_global.unsqueeze(2).repeat(1, 1, tgt_feats.shape[2]),
                tgt_feats,
                src_global.unsqueeze(2).repeat(1, 1, tgt_feats.shape[2])
            ], 1)
            output_states = src_feats + self.cross_layer(src_cross)
