...
from glorn.modules.attention.rpe_transformer import GeometricSelfAttention
from glorn.modules.attention.vanilla_transformer import TransformerLayer
from glorn.modules.attention.vanilla_transformer import Overlapping
...
class OverlappingFactor(nn.Module):
    def __init__(
        self,
        blocks,
        d_model,
        ...
        super(OverlappingFactor, self).__init__()
        self.blocks = blocks
        layers = []
        for block in self.blocks:
            _check_block_type(block)
            if block == 'self':
                layers.append(GeometricSelfAttention(d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
            else:
                layers.append(Overlapping("global patch", d_model, num_heads, dropout=dropout, activation_fn=activation_fn))
        self.layers = nn.ModuleList(layers)
        self.return_attention_scores = return_attention_scores
        self.parallel = parallel
