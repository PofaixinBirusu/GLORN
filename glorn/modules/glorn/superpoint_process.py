...
from glorn.modules.ops import pairwise_distance
from glorn.modules.attention import SinusoidalPositionalEmbedding
from glorn.modules.attention import OverlappingFactor
...
class SuperPointProcessor(nn.Module):
    def __init__(
        self, input_dim, output_dim, hidden_dim, num_heads, blocks, sigma_d, sigma_a, angle_k,
        dropout=None, activation_fn='ReLU', reduction_a='max',
    ):
        super(SuperPointProcessor, self).__init__()

        self.embedding = GeometricStructureEmbedding(hidden_dim, sigma_d, sigma_a, angle_k, reduction_a=reduction_a)

        self.in_proj = nn.Linear(input_dim, hidden_dim)
        self.overlapping_factor = OverlappingFactor(
    ...
    def forward(self, ref_points, src_points, ref_feats, src_feats, ref_masks=None, src_masks=None):
        ...
        ref_feats, src_feats = self.overlapping_factor(
            ref_feats,
            src_feats,
            ref_embeddings,
            src_embeddings,
