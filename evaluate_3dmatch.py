...
from dataset_3dmatch import test_data_loader
from config.config_3dmatch import make_cfg
from model_3dmatch import create_model
...
def make_parser():
    ...
    parser.add_argument('--snapshot', default="./params/GLORN-3DMatch.pth.tar", help='load from snapshot')
    return parser
...
def main(test_benchmark_name):
    global benchmark
    benchmark = test_benchmark_name
    cfg = make_cfg()
    tester = Tester(cfg)
    tester.run()
